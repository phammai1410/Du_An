...
# Files required to run Qwen/Qwen3-Embedding-0.6B with the TEI backend
REQUIRED_FILES = (
    "config.json",
    "model.safetensors",
    "tokenizer.json",
...
def resolve_target_dir(custom_target: str | None = None) -> Path:
    ...
    Defaults to backend/local-llm/embedding/Qwen-Qwen3-Embedding-0.6B relative to this file.
    """
    if custom_target:
        ...
        target_path = (
            ...
            .resolve()
            .parents[1]
            .joinpath("local-llm", "embedding", "Qwen-Qwen3-Embedding-0.6B")
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download required Qwen/Qwen3-Embedding-0.6B files for TEI backend."
    )
    parser.add_argument(
        "--target",
        help=(
            "Custom directory to place the downloaded files. "
            "Defaults to backend/local-llm/embedding/Qwen-Qwen3-Embedding-0.6B."
        ),
    )
