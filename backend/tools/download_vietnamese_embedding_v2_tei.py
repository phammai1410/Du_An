...
# Files required to run AITeamVN/Vietnamese_Embedding_v2 with the TEI backend
REQUIRED_FILES = (
    "config.json",
    "model.safetensors",
    "tokenizer.json",
...
def resolve_target_dir(custom_target: str | None = None) -> Path:
    ...
    Defaults to backend/local-llm/embedding/AITeamVN-Vietnamese_Embedding_v2 relative to this file.
    """
    if custom_target:
        ...
        target_path = (
            ...
            .resolve()
            .parents[1]
            .joinpath("local-llm", "embedding", "AITeamVN-Vietnamese_Embedding_v2")
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download required AITeamVN/Vietnamese_Embedding_v2 TEI files into the local embedding directory."
    )
    parser.add_argument(
        "--target",
        help=(
            "Custom directory to place the downloaded files. "
            "Defaults to backend/local-llm/embedding/AITeamVN-Vietnamese_Embedding_v2."
        ),
    )
