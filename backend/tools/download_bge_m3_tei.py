...
# Files required to run BAAI/bge-m3 with the TEI backend
REQUIRED_FILES = (
    "config.json",
    "pytorch_model.bin",
    "tokenizer.json",
...
def resolve_target_dir(custom_target: str | None = None) -> Path:
    ...
    Defaults to backend/local-llm/Embedding/BAAI-bge-m3 relative to this file.
    """
    if custom_target:
        ...
        target_path = (
            ...
            .resolve()
            .parents[1]
            .joinpath("local-llm", "Embedding", "BAAI-bge-m3")
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download required BAAI/bge-m3 TEI files into the local embedding directory."
    )
    parser.add_argument(
        "--target",
        help="Custom directory to place the downloaded files. Defaults to backend/local-llm/Embedding/BAAI-bge-m3.",
    )
